...
final class YOWM_Student_Access {
	...
	public static function handle_save_roster(): void {
		...
		foreach((array)($_POST['students']??array()) as $uid=>$row){$uid=absint($uid);$u=get_userdata($uid);if(!$u||!in_array(self::ROLE,(array)$u->roles,true))continue;wp_update_user(array('ID'=>$uid,'first_name'=>sanitize_text_field(wp_unslash($row['first_name']??'')),'last_name'=>sanitize_text_field(wp_unslash($row['last_name']??'')),'display_name'=>trim(sanitize_text_field(wp_unslash($row['first_name']??'')).' '.sanitize_text_field(wp_unslash($row['last_name']??'')))?:$u->user_login));self::set_memberships($uid,array_map('absint',(array)($row['cohorts']??array())));}
